...
# tmp_tab <- read_sheet("https://docs.google.com/spreadsheets/d/1SSvav_ISY1h0N7zr6xgxSyrQV8_XpdpsIUYEWzikaCA/edit#gid=0", sheet = 2)
# 
# 
# tab %>% select(Reference) %>% rbind(tmp_tab %>% 
#   filter(Metric == c("SR", 
#                      "Functional richness",
#                      "Evenness", 
#                      "Functional evenness",
...
#     select(Reference)) %>% distinct(Reference)
...
tab <-
  tab %>%
    ...
    mutate(`Spatial grain (Km²)` = case_when(
      `Spatial grain (Km²)` <= 650 | `Spatial grain (Km²)` == "Local" ~ "Local",
      (`Spatial grain (Km²)` > 650 & `Spatial grain (Km²)` < 800000) | (`Spatial grain (Km²)`== "Admin. unit") | (`Spatial grain (Km²)`== "Regional")~ "Regional",
...
tab %>% 
  select(Reference_, `Temporal extent (year)`, Years) %>% 
  separate(Years, c("start", "end"), "-") %>% 
  mutate_at(vars("start", "end"), as.numeric) %>% 
  filter(!duplicated(Reference_)) %>% 
  mutate(Reference_ = fct_reorder(Reference_, `Temporal extent (year)`)) %>% 
...
tab %>%
  select(Reference, `Spatial grain (Km²)`, `Temporal grain (hour)`, Trend, Note) %>%
  as_tibble() %>%
  group_by(Reference) %>%
  collapse_rows_df(Reference) %>%
  flextable::flextable() %>%
