...
tab <-
  ...
  filter(!(Reference == "@jarzyna_taxonomic_2018" & `Spatial grain (Km²)` == "National")) %>% 
...
# tab <- tab %>% select(-Reference) %>% relocate(Reference_) %>% rename(Reference = Reference_)
...
ggdraw()+
  draw_plot(Fig3a_noreplic, x = 0, y = .5, width = .3, height = .5)+
  draw_plot(Fig3b_noreplic, x = .3, y = .5, width = .7, height = .5)+
  draw_plot(Fig3c_noreplic,  x = 0, y = 0, width = 1, height = 0.5)+
  draw_plot_label(label = c("a", "b", "c"), size = 13,
                  x = c(0, 0.29, 0), y = c(1, 1, 0.5))
...
tab_notes %>%
  ...
  mutate(`Spatial extent (Km²)` = tab_notes$`Spatial extent (Km²)`,
...
tab_notes <-
## Take off the abundance metrics and the non replicated ones at national scales (except for America because it is a huge scale)
  tab_notes %>% filter(Metric != "Abundance") %>% 
  filter(!(Reference == "@reif_changes_2013" & `Spatial grain (Km²)` == "National"),
         !(Reference == "@van_turnhout_scale-dependent_2007" & `Spatial grain (Km²)` == "National")) %>% 
...
# tab_notes <- tab_notes %>% select(-Reference) %>% relocate(Reference_) %>% rename(Reference = Reference_)
```
