...
* Reducing the spatial scales to fewer levels: so far, Local <= 25 Km², Regional > 25 Km² and Continental for the sizes of USA and worldwide. 
...
# tmp <-
  tab %>%
    ...
    mutate(`Spatial grain (Km²)` = case_when(
      ...
      (`Spatial grain (Km²)` > 25 & `Spatial grain (Km²)` < 800000) | (`Spatial grain (Km²)`== "Admin. unit") | (`Spatial grain (Km²)` == "National") | (`Spatial grain (Km²)`== "National unit") ~ "Regional",
      TRUE ~ "Continental"
    )) %>% 
...
```{r, echo=FALSE}
  ...
  knitr::kable(tmp %>% 
                 filter(`Spatial grain (Km²)` == "Continental") %>% 
                 # ungroup() %>%
                 select(-2),
               caption = "Continental") %>% 
    column_spec(1:4, width = "6.5em")
